...
Everything that is no longer a live question. `PLAN.md` carries what is.

Two halves, and they are different kinds of document. **Closed questions** below
are distilled: the conclusion and the mechanism behind it, in the tense of a
settled fact. **The investigation log** after them is the former `PLAN.md`
frozen verbatim -- two weeks of working notes, chronological, present tense even
where the tense is now wrong, kept because the measurements and the readings that
were later withdrawn are the evidence the distillations were cut from.

**Do not append to the log half.** New closures go in the top half, newest at the
bottom of it. Where the two halves disagree, the top half wins.
...
## P4. CLOSED -- cheap diagnostics that plug into someone else's stack

`diagnostics_enabled` plus `pop_diagnostics()`: a plain dict of floats, no
trainer assumed, opt-in, one attribute read per accumulation site, measurements
on-device with the single host read at drain, so a drained point is the
interval's mean. Verified with CUDA sync debug mode -- five steps cost 25
synchronizing operations with telemetry off and 25 with it on. Both consumers are
wired. Metric table lives in `SPEC.md`.

What was deliberately not ported from the lab's set: `capture` (a weak proxy, and
measuring it on the raw gradient would cost an extra projection per matrix per
step), `aurora_erank` / `aurora_alignment` / the leverage statistics (cost, and
P1's reasons), and raw `update_norm`, demoted to `update_to_param_ratio` -- the
only form of it that ever said anything.

---

## P7. CLOSED -- bf16 basis storage costs 0.6% of the moment's smear. Keep bf16.

`transport_spin` measures a numerical property rather than a loss delta, so a
synthetic gradient is admissible here: the question is what rounding does to a
frame. An ideal geodesic has zero spin (`Q^T Q+ = V cos(theta) V^T`, exactly
symmetric). fp32 reads `7e-8` at window 1 growing linearly -- genuine holonomy.
bf16 reads `5.6e-4` at window 1 where exact arithmetic gives zero, and `spin^2`
is linear in the window with **zero intercept**: a pure random walk, no
measurement floor. So bf16 does inject real in-span rotation, and in-span
rotation is the one motion that scrambles the projected moment's coordinates for
no subspace progress.

It is immaterial. Over the moment's memory at `beta=0.9`, spin is `0.0018`
against `0.0194` of legitimate smear; in quadrature, 1.936% to 1.948%. Against
+50% of matrix optimizer state, not a trade worth making. Stochastic rounding on
...
**A per-role rank table beats one global rank on both heads at equal budget.**
LFM bs16, 1k, seed 1: target `1.669330 -> 1.667302`, source `3.084276 ->
3.079098`, both clear of the floor, at +2.2% planes. Both heads improving is what
makes it a Pareto gain rather than a walk down P11's effective-LR axis, and
`tangent_participation` up 21% says what changed -- the tracker resolves a
better-conditioned subspace, which no learning rate buys. A global rank gives
attention roughly twice the rank its gradients can fill while starving the MLP by
a third.

**Role is the right grouping.** Per-matrix mean `live_fraction` is explained
73.6% by role, 58.4% by shape, 19.1% by depth. Role beats shape rather than
proxying for it: `k_proj` and `v_proj` are both (512,1024) and read `0.711`
against `0.429`.

**The live set saturates, and rank is a target/source dial.** At bs1 the live
count goes `31.2 -> 51.2 -> 60.2` across ranks 32/64/128 -- near-proportional to
64, saturating past it. **Effective planes are invariant to tracked rank**
(`1.50 / 1.55 / 1.57` across a 4x range): the aim carries about one and a half
planes whatever the frame's size. Rank does not change what the gradient offers,
only how many planes are positioned to see it. Filling the interior puts the
target optimum at `live_fraction` ~0.90 and the balanced setting at 0.95, which a
200-300 step diagnostic reads directly.

**The liveness floor is not load-bearing and cannot substitute for rank.** Swept
`LIVE_FLOOR_SIGMAS` over a 16x range in `eps` at two ranks: target spans `3.5e-4`
against a `3e-4` floor, and `participation` / `concentration` are invariant to
five decimals -- the control that says the knob changes the response to the aim
and not the aim. It stays at the derived `1.0`. r128 with the floor raised to
match r32's live count still loses to r32 by `4.3e-3`, and the two knobs move
churn in opposite directions, because muting a plane stops it rotating but leaves
its ballast in the update subspace.

---

## CLOSED -- the Muon aspect factor is not double duty with the rank table

Three scale rules at matched effective step (`update_to_param_ratio` ~`1.64e-4`),
LFM bs16, 300 steps: shipped `muon` `sqrt(max(1, rows/cols))` reads
`1.7062 / 3.0424`, `unit` `1.0` reads `1.7095 / 3.0509`, `fullrank`
`sqrt(min(m,n)/r)` reads `1.7118 / 3.0504`. `muon` wins **both** heads. A first
pass at one LR only measured the effective-step curve and had to be redone -- the
losses tracked P11's axis, not the scale shape.

**Orthogonal levers.** The table sets which subspace and how many directions; it
is the table, not the scale, that moves `live_fraction` and `participation` (the
geometry panel is flat across all five arms, so the scale rule is a pure
step-size *distribution* lever). The aspect factor sets how hard the fan-out roles
push. The `[m,n]`-shape formula transfers to the subspace intact -- the update is
still an `R^n -> R^m` map. *Caveat:* in LFM2's MLP role and shape are collinear,
so this cannot separate "boost fan-out shapes" from "boost up/gate roles".

The dispatch behind it is also gone: one `_orthogonalize_update(update, scale)`
that `torch.compile` wraps, so the compiled and eager paths cannot disagree --
they previously did, for any mode but `muon`, because the compiled kernel
hardcoded the muon scale and never read the constant.

---

## CLOSED -- tracking the residual-facing side, on loss, against its own geometry

The `side=right` arm captured 15% more gradient, spread the spectrum best of any
arm measured all session, and lost on **both** heads -- target by `8.2e-3`, 27x
the noise floor.
...
self-contained and therefore more stable, which is exactly why it tracks better.
**Trackability and usefulness are anti-correlated here.** The optimizer should
track the side that is harder to track, because that is where the model's
coupling lives.

*A correction recorded because it was stated and is wrong.* It was argued that
tracking the larger side is structurally wasteful -- that a `[1024, r]` frame on
a (512,1024) matrix spans dimensions the gradient can never populate. False: the
frame is fitted, so Oja and the initial `eigh` place it inside the gradient's own
row space. What the side genuinely changes is frame memory and semantics.
---

## CLOSED -- the controller is as derived as it gets, and `eta` is calibratable

The programme was to remove fitted constants from the step rule, and it ran to
completion. The fitted `2.87` became a running peak, the running peak became a
fleet reading, and the floor became a derivation from `k`, `d` and `r`. The
ceiling is now computed every step as the fleet's effective aim rank over the
meter width -- `median participation * r / k = 0.1413` against a median observed
peak of `0.1495`, 6% apart -- so the gain is not a constant to fit. It also
*moves*, rising ~47% across a 1k run as participation grows, which is exactly
...
moves the turn scale 4.3x while moving target loss by less than the same-config
spread -- a redundancy of the same kind the fitted `2.87` was. The honest count
is one-and-a-half constants, not one. It is recorded as accepted rather than
resolved: nothing tried beat the shipped meter, and collapsing the pair would
buy tidiness rather than behaviour.

---

## CLOSED -- the agreement meter is shipped, not pending

An open lead read "agreement `cos(Z, M)` as a step-size input: instrumented and
understood but not used." That was stale about its own codebase. The top-`k`
agreement meter with the fleet gain is the shipped controller -- `scale =
clamp(excess / G, 0, 1)`, one yardstick for the fleet, so matrices that hold
their aim longer keep turning rather than having their differences normalized
away. Per-matrix ceilings were tried and are worse (see "Already tried").

---

## CLOSED -- horizon does not break the tracker

The prediction was that a 300-step comparison favours fast trackers, so a
slower-net-travelling frame should pull ahead by 1k. It did not: the gap moved
the other way by `2.6e-4`, under the harness noise bar, and frozen `sigma`
saturates by step 200, so the geometric gap is bounded.

Longer runs answered the safety half rather than the value half. A 3600-step bs4
run and a 2304-step Anima finetune both hold: no degradation, no drift in the
suggested rank table across windows 2-12, frame motion settling normally. **The
aim is bad at small batch and stable there** -- at bs1/bs4 effective rank is ~1.5
planes and flat across three epochs, which is a property of the gradients at that
batch size rather than something training fixes or something that decays. Bad
aim, not deteriorating aim.

---

## CLOSED -- masking the update to the live planes is unnecessary

The arm was: rank is the subspace the update is orthogonalized in while the live
floor is only the subspace the turn acts on, so a muted plane still carries its
ballast in the update. If rank's whole advantage were "do not orthogonalize in
directions you cannot resolve", a fixed rank with a live-masked update should
recover low-rank behaviour without a rank decision.

The per-role rank table does that job directly and statically. Module liveness
barely moves over a run -- suggested ranks are identical across windows 2-12 of a
3600-step run -- so a dynamic mask would track a constant the table already
encodes, at the cost of a per-step branch on the update path. Not built.

---

## CLOSED -- Oja tracking beats a fresh eigendecomposition, and the arc was wrong

The redesign arc's centerpiece argued Oja is open-loop velocity control that
random-walks on noise, and that EIGH-aimed position control is strictly better.
The refit control measures the opposite on the current mechanism: a fresh
eigendecomposition mid-training is *worse* than the tracked frame, and the margin
is clearest with the orthogonalized tangent, where every plane turns by the same
angle rather than the leading plane's magnitude dominating the turn. Tracking
carries information a per-step refit throws away. Superseded, not pending.

---

## CLOSED -- dynamic rank, given up

The hoped-for shape was expand and reduce in one motion at a refit interval,
adding no new state or windows. It does not close on its own terms: new rows
initialized to zero or noise cannot be read for liveness, because the reading
needs a frame difference. And the static table removed the motive -- it is
identical across windows 2-12 of a 3600-step run, so a controller would be
tracking a constant while adding state, a window, and a failure mode.

*Recorded because it is cheap and someone will ask.* Pruning alone needs no
refit and does not reset the moment: rotate `Q -> QV` into the tangent eigenbasis
(orthogonal, so the subspace does not move), apply the same `V` to the moment,
truncate both. That rotation is what `transport_spin` measures and transport is
the identity in those coordinates. The decomposition is already computed every
basis update, so prune is free at any step; only *growing* needs new orthogonal
directions. `grad_to_moment_ratio` is invariant to the rotation and not to the
truncation, so it steps at each prune event.

---

## CLOSED -- the parked items, none of which came back

**Parameter eligibility and routing.** The exclusion rules -- lookup tables,
multiplicative gates -- stay in the ai-toolkit adapter, which exists for good
reasons. A library-side `RoutingPolicy` was prototyped and parked on branch
`study/rotation-clamp-eligibility`; nothing since has wanted it, and the side
question it was partly for is now answered by the `build_usuitrack_param_groups`
heuristic. Show a concrete alternative before arguing for one.

**`basis_update_interval != 1`.** The default is per-gradient motion. A larger
interval is a distinct estimator-variance and systems trade -- fewer geodesics
run, and a larger harmonic move is not automatically equivalent to several small
ones. Never measured, and nothing in the current design wants it.

**Full-spectrum rotation vs rank-1 drift.** Framed as a way to avoid spinning the
frame on the near-isotropic noise tail. Answered from the other side: the tail is
not the problem -- turning only the top planes was *worse* than baseline, while
orthogonalizing the tangent so every plane turns by the same angle was better.
The leading plane's magnitude dominating the turn was the problem, and the
shipped controller addresses it. Superseded.

**`nan_to_num` placement.** Moot: both the tangent `nan_to_num` and the jittered
`eigh` retry are deleted. Across four LFM 1k runs and a 2304-step Anima
finetune -- two models, two architectures, two ranks -- `nonfinite_grads` read
**0** and `eigh_jitter_retries` never fired once. A failing `eigh` now fails,
because its result steers the frame and a silently rescued decomposition is worse
than a stopped run. Two guards deliberately survive and are not covered by that
evidence: the raw-gradient sanitize at the clip (a different thing at a different
place -- it is what stops a bad batch reaching any consumer) and
...
still stands has been restated above.

---

## P1. CLOSED -- Adafactor is gone

Not needed by either consumer; deleted from the release (state, FLOPs, both
`adafactor_*` arguments, two fused kernels, the `row_mute_fraction` diagnostic).
It bought `0.0035` target loss for 2.5% walltime and 1.5% state, and with it the
tracker could not converge at any step size -- a congruence does not preserve the
eigenspace the frame is fitted to. Locked in by
`test_a_fitted_frame_is_a_fixed_point_of_its_own_gradient`. Confirmed clean on a
2304-step Anima finetune. Full write-up in `ARCHIVE.md`.
...
The hump was hypothesised to end at `1/(1 - adafactor_beta2) = 100`. No factored
variance any more, so nothing to verify. A hump in the current tracker would be a
new observation. In `ARCHIVE.md`.
